...
//     `https://free-to-play-games-database.p.rapidapi.com/api/games?category=${gameGenre}`,
//     options
//   );
...
//     cartona += `<div class="col-lg-3 col-md-4 col-sm-6 position-relative showInfo" data-gameId = "${
//       response[i].id
//     }">
...
//                   </div>
//                   <p class="card-text text-secondary text-center">${response[
//                     i
//                   ].short_description
//                     .split(" ")
//                     .slice(0, 7)
...
//                   <small class="p-0 my-1 gameTypeColor fw-bolder rounded-2 px-2">${
//                     response[i].genre
//                   }</small>
//                   <small class="p-0 my-1 gameTypeColor fw-bolder rounded-2 px-2">${
//                     response[i].platform
...
class GameList {
  constructor() {
    this.gameLinks = document.querySelectorAll(".gameLinks");
    this.gameContainer = document.querySelector(".gameContainer");
    this.home = document.getElementById("home");
    this.gameData = document.getElementById("gameData");
    this.gamesArr = [];
    this.gameDetails = new GameDetails(this.home, this.gameData);
    this.setupLinks();
    this.setupCardClick();
  ...
  setupLinks() {
    for (let i = 0; i < this.gameLinks.length; i++) {
      this.gameLinks[i].addEventListener("click", (e) => {
        let gameGenre = this.gameLinks[i].innerHTML;
        this.displayGames(gameGenre);
      });
    }
  }

  async displayGames(gameGenre) {
    const options = {
      method: "GET",
      headers: {
        "x-rapidapi-key": "f8bc025352mshcb9bd43f165a006p1f82c0jsndae2fffbcbd2",
    ...
    this.gameContainer.innerHTML = cartona;
  }
  ...
  setupCardClick() {
    this.gameContainer.addEventListener("click", (e) => {
      ...
      if (card == null) return;
      let id = card.getAttribute("data-gameId");
      this.gameDetails.getGameDetails(id);
    });
  }
}

class GameDetails {
  constructor(home, gameData) {
    this.home = home;
  ...
  async getGameDetails(gameId) {
    let req = await fetch(
      `https://free-to-play-games-database.p.rapidapi.com/api/game?id=${gameId}`,
      {
        ...
        headers: {
          "x-rapidapi-key":
            "f8bc025352mshcb9bd43f165a006p1f82c0jsndae2fffbcbd2",
          "x-rapidapi-host": "free-to-play-games-database.p.rapidapi.com",
        },
      }
    );
    ...
    this.showDetails(gameDetails);
  }

  showDetails(gameDetails) {
    this.home.classList.add("d-none");
    this.gameData.classList.remove("d-none");

    document.querySelector(".gameDetailsContainer").innerHTML = `
      <div class="col-md-4">
        <img src="${gameDetails.thumbnail}" class="w-100" alt="">
      </div>
      <div class="col-md-8 ">
    ...
    closeX.addEventListener("click", () => {
      ...
      this.gameData.classList.add("d-none");
    });
  }
}
new GameList();
